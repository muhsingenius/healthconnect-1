/*
  ...
  1. Sample Questions
    - Creates 8 diverse health questions across different categories
    - Mix of anonymous and public posts
    - Realistic content with proper tags and categories

  2. Sample Answers
    - 7 professional answers from verified doctors
    - 1 community answer from a patient
    - Realistic medical advice and engagement

  3. Engagement Data
    - Question upvotes with realistic patterns
    - Answer upvotes showing community engagement
    - Proper timestamps and view counts
*/
...
-- Store question IDs for answers (using a temporary table approach)
DO $$
DECLARE
    q1_id uuid;
    q2_id uuid;
...
BEGIN
    ...
    SELECT id INTO q1_id FROM questions WHERE title = 'What are the early symptoms of diabetes?';
    SELECT id INTO q2_id FROM questions WHERE title = 'How often should I get a general health checkup?';
    SELECT id INTO q3_id FROM questions WHERE title = 'Best foods for managing high blood pressure?';
    SELECT id INTO q4_id FROM questions WHERE title = 'How to manage anxiety and stress naturally?';
    SELECT id INTO q5_id FROM questions WHERE title = 'Essential nutrients during pregnancy?';
    SELECT id INTO q6_id FROM questions WHERE title = 'Safe exercises for people with joint pain?';
    SELECT id INTO q7_id FROM questions WHERE title = 'Trouble sleeping - when should I see a doctor?';
